// [DY INTEGRATION] Server-side proxy for Dynamic Yield Visual Search.
// Mirrors api/visual-search.ts from sinsay_v2. NovaPower customers can upload
// a photo of a router, smart device, EV charger, etc. and DY returns visually
// similar items from the product feed.
//
// POST /api/visual-search → forwards to dy-api.com Visual Search endpoint with API key
export default async function handler(req: any, res: any) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const { imageBase64, imageUrl, pageType, pageData, apiKey: bodyApiKey } = req.body;
  ...
  const apiKey = (typeof bodyApiKey === 'string' && bodyApiKey.trim()) ? bodyApiKey.trim() : process.env.VISUALSEARCH_API_KEY;
  if (!apiKey) {
  ...
  try {
    ...
    const payload = {
      ...
      query: {
        sortBy: { order: 'asc', field: 'popularity' },
        imageBase64: resolvedBase64,
      },
      context: {
        page: {
          type: pageType || 'HOMEPAGE',
          location: 'https://novapower.demo',
          data: Array.isArray(pageData) && pageData.length ? pageData : ['novapower-home'],
        },
      },
      selector: { name: 'Visual Search' },
    ...
    return res.status(200).json({ results, totalResults, rawResponse: data });
  } catch (error) {
...
async function fetchImageUrlAsBase64(imageUrl: string): Promise<string> {
  ...
  if (arrayBuffer.byteLength > maxBytes) throw new Error('Image size exceeds 10MB limit');
...
function extractVisualSearchResults(response: any): any[] {
  try {
    ...
    if (Array.isArray(response.choices)) {
      for (const choice of response.choices) {
        ...
        for (const variation of variations) {
          ...
          if (payloadData && Array.isArray(payloadData.slots)) slots.push(...payloadData.slots);
        }
      }
    }
    return slots.map((slot) => {
      ...
      if (slot.productData) {
        return { ...slot.productData, sku: slot.sku ?? slot.productData.sku, slotId: slot.slotId };
      }
      return slot;
    });
  } catch (error) {
...
function extractVisualSearchTotalResults(response: any): number {
  ...
  if (typeof firstResponse?.totalNumResults === 'number') return firstResponse.totalNumResults;
  if (typeof response.totalNumResults === 'number') return response.totalNumResults;
  return 0;
}
